...
;; 6 breeds needed in total
;; 3 for the slime mold (plasmodia, pseudopodia, tubes)
;; 1 for food source
;; 2 for A* algorithm ( networkpoints, searchers)
...
globals [
  ;; To control the shape of the visible chemical field
  scale-factor
  ;;Ddetermines the probability with which pseudopodia hatch a new pseudopodium
  hatch-probability
  ;; Stores the position of the plasmodium
  plasmodium-position
]

pseudopodia-own[
  ;; Stores the path in a list with x-y coordinates
  path-list
]

foods-own [
  ;; Each food source should contain a certain amount of nutrients
  nutrient-value
...
searchers-own [
  memory               ; Stores the path from the start node to current node
  cost                 ; Stores the real cost from the start
  total-expected-cost  ; Stores the total expected costs from start to finish that are currently being calculated
  localization         ; The searchers position
  active?              ; Is the searcher active? That is, the node has been reached, but it must still be considered because its neighbors have not yet been explored.
]
...
to setup
  ...
  ;; Responsible for the visibility of the chemical concentration in the air
  ask patches [ generate-field ]
  ;; Sets the initial position of the plasmodium in the center
...
to go
  ...
  [
      ...
      [
        ...
        [
          ...
          if nutrient-value = 0 [
            ...
            ;; The plasmodium-position property shows the current position of the plasmodium.
    ...
    ;; If the A* has formed a tube, it should be displayed.
    ask tubes[
      let i 0
...
to look-for-food
  ;; Movement to the food by simulated chemotaxis
...
to bounce
  ;; When "at the end of the world" look in the middle and go forward one field
  ;; Move one field forward, otherwise it will be stuck at the edge of the worldworld
